...
.. attention:: La conexión a una base de datos es una **operación costosa**, por
   lo que, si la aplicación abre y cierra constantemente conexiones, el
   rendimiento se penalizará. Lo habitual en una aplicación que sea algo más que
   una prueba de concepto, es utilizar un :ref:`pool de conexiones <conn-pool>`,
   el cual es capaz de reaprovechar las conexiones. Lo trataremos más adelante.
...
#. Las sentencias |SQL|, que son sentencias |SQL| cuya comprensión no
   ...
   .. note::
      ...
      No obstante, sí conviene precisar que procuremos al escribirlas
...
.. hint:: El objeto |ResultSet| hay también que cerrarlo, aunque debería
...
.. warning::
   ...
   El método es de ejecución perezosa, esto es, genera objetos a
...
.. tip:: Si no nos interesa cerrar la conexión al cerrar el flujo, podemos pasar
   ``null`` en el argumento correspondiente.
