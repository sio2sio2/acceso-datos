...
usa un `patrón Multiton`_, de manera que si intentamos crear un pool con la
misma clave, genera una excepción:

.. code-block:: java

   ConnectionPool pool1 = ConnectionPool.create("BD", "jdbc:sqlite:file::memory:?cache=shared");
   ConnectionPool pool2 = ConnectionPool.create("BD", "jdbc:sqlite:datos.db");  // IllegalStateException
   ConnectionPool pool3 = ConnectionPool.get("BD");  // Recuperamos el pool.
   pool1 == pool3; // true

.. caution:: Obsérvese la |URL| que se ha utilizado en los ejemplos. En el caso
   particular de *SQLite* y HikariCP_, para que las distintas conexiones del *pool* abran
...
por lo que resulta muy útil construirse algún mecanismo más cómo para llevarlas
...
.. caution:: La clase presupone que hemos definido una excepción protegida de usuario
   ...
   <DataAccessRuntimeException>` que captura los errores propios de la
   persistencia (``SQLException`` en nuestro caso).
...
Este gestor de transacciones soporta múltiples hilos y el acceso a mútiples bases de datos:
