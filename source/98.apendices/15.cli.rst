...
.. caution::
   ...
   En *Java*, los elementos de este array incluyen (en orden) exclusivamente los
   argumentos del programa, por lo que el primer elemento (el elemento 0)
   también es argumento y no el nombre con el que se invocó el programa, como
   ocurre en otros lenguajes como *Python* o *C*.
...
cumpla con el :ref:`estándar POSIX <linux:ordenar>`. Para ello, hay distintas
...
#. Añadir en :file:`pom.xml` un sección que añada el archivo al proyecto y
   ...
   .. caution:: En ausencia de sección :code:`<resources>` *Maven*, por defecto,
      incorpora a la compilación todos los archivos que se incluyan dentro de la
      ruta predefinida :file:`src/main/resources`. pero sin realizar sustitución
      de variables.  Como nuestra intención es que la haya, necesitamos
      expresamente declarar la incorporación incluyendo la línea resaltada.
      Ahora bien, añadir esta sección de recursos, deshabilita este
      comportamiento predefinido y provoca que entonces sólo se incluya lo
      explícitamente declarado en la sección: en este caso, cualquier archivo de
      extensión :file:`.properties` dentro del subárbol
