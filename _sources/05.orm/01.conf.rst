...
* El soporte para |JPA| que proporciona Hibernate_ mediante hibernate-core_.
  ...
  `jakarta.persistence-api`_ (con la prevención que más adelante se refiere).
...
* En principio, basta con lo anterior, pero si se quiere llevar a cabo
  ...
  adelante <orm-mapping-check>`), será necesaria la implementación para la
  especificación |JPA| que proporciona *Hibernate*, esto es, hibernate-validator_.
  Como anteriormente, podemos incluir explícitamente la implementación
  (`jakarta.validation-api`_).
* La última dependencia (`expressly`_) sólo es necesaria si se utilizan
...
* Las propiedades (dentro de ``<properties>``) pueden alterarse dinámicamente
  durante la ejecución del programa antes de conectar a la base de datos. De
  hecho, no es conveniente que se incluyan en el archivo, sino que se
  :ref:`añadan dinámicamente <orm-conf-dyn>`.
...
* El proveedor indica cuál es la implementación |JPA| que usaremos. Algunos
  ...
  En cualquier caso, el elemento ``<provider>`` no puede :ref:`cambiar de valor
  dinámicamente <orm-conf-dyn>`. Si queremos otra configuración alternativa para
  un |ORM| distinto, necesitamos crear una unidad de persistencia distinta.
...
* Las propiedades son de dos tipos:
  ...
  - Las propias de |JPA| y que son independientes de la implementación
    utilizada.
  - Las propias de la implementación concreta. En el ejemplo, la implementación
    es Hibernate_.
...
las propiedades y pasar el mapa cuando se usa la unidad de persistencia para
crear el objeto :jakarta-persistence:`EntityManagerFactory` (véase a
continuación :ref:`cómo conectar <orm-bas-conn>`). En ese caso, las
definiciones del mapa sobrescribirán a las que pudieran existir en el archivo.
Por ejemplo:
...
.. caution:: Es necesario que el archivo
