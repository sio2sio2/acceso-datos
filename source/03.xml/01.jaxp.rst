...
.. admonition:: Advertencia
   ...
   Con este código se producirá un error si el |XML| no es bien
...
``getTextContent()``
   ...
   .. code-block:: java
      ...
      apelativo.getTextContent().equals(apelativo.getFirstChild().getNodeValue());
...
``getChildNodes()``
  ...
  .. tip::
     ...
     Pese a que, a priori, sólo podemos utilizar el `for` tradicional
...
.. admonition:: Advertencia
   ...
   *precisamente la forma en la que hemos ilustrado el uso*) no hay modo de
   conocer su valor y, en consecuencia, será ``null``. Eso sí, en este último
...
.. tip::
   ...
   El argumento del constructor de `InputSource` también puede ser un
   ...
   encuentre), podemos hacer lo siguiente:
