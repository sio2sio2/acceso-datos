...
Así pues, el propósito a seguir cuando se codifica una aplicación es que todas las particularidades del acceso a datos estén reducidas a un paquete dentro de la aplicación (p.e. *backend*), fuera del cual no haya otra cosa que objetos.
...
últimas relacionan la aplicación con la base de datos, de modo que el acceso a
estas se hará exclusivamente a través de ellas. En la aplicación, por tanto,
jamás haremos referencia a un objeto ``Connection`` o prepararemos una sentencia
|SQL|. Eso sí, necesitamos también una clase que gestione la conexión a la base de datos y que se encargue de:

.. _rst-class: simple

#. Crear el pool de conexiones con el |DataSource| apropiado.
#. Realizar labores de inicialización como, por ejemplo, si la base de datos está
   vacía, poblarla con el esquema.
#. Proporcionar al resto de la aplicación los dos objetos |DAO| asociados a la
   persistencia de centros y estudiantes.
...
.. code-block:: java
   ...
   try {
      transaccion( (centroDao, estudianteDao) -> {
         ...
         // todas están en la misma transacción.
      });
   }
   catch(DataAccessException err) {
      // Tratamiento del error (el rollback ya se ha hecho).
   }
...
La implementación es sencilla de entender: se crea un pool de conexiones al crear el objeto, se puebla la base de datos en caso de no existir previamente; y, con los métodos correspondientes, se devuelven instancias de las clases ``CentroSqlite`` y ``EstudianteSqlite`` a las que se ha pasado el |DataSource| para su construcción. Por tanto, los objetos |DAO| generados no podrán usarse para crear transacciones con varias operaciones. El código menos trivial es el método ``transaccion`` que permite escribirlas del modo en que :ref:`se dejó escrito arriba <conn-transaccion-impl>`:
...
por lo que sus operaciones podrán formar parte de una transacción, y que son los
que recibe como argumento la :ref:`función lambda que debe escribirse en el código del programa <conn-transaccion-impl>`. El por qué constituyen una transacción es causa de la implementación de ``transactionSQL`` de sqlutils_.
...
.. warning:: Como el método ``transaccion`` crea su propia conexión, no es
   ...
   de otro modo las transacciones (porque ``transactionSQL`` sí es anidable).
...
Hecho eso, la relación con la base de datos está totalmente encapsulada. Échele un
vistazo a `cómo realizar operaciones de almacenamiento en el ejemplo de prueba
Test.java
