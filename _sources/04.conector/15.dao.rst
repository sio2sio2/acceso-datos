...
<clase-connectionpool>` ) (o una clase más específica de nuestra aplicación que
...
|DataSource| que nos interesa; y, como ya adelantamos, la conexión la obtenemos
...
.. code-block:: java
   ...
   DataSource ds = ConnectionPool.createHikariDataSource(dbUrl, dbUser, dbPassword);
   TransactionManager tm = TransactionManager.create("MiBase", ds);

   // Este DAO ya sabe con qué gestor de transacciones está relacionado.
   CentroDao centroDao = new CentroDao("MiBase");

   // Creamos una transacción y con ello una conexión.
   tm.transaction(conn -> {
...
.. note:: En realidad, nuestro diseño sería más limpio si crearamos una clase
   ``Conexion`` que:

   + Implementara un patrón Multiton_ para asociar claves a objetos |DataSource|
     creados con ``ConnectionPool``.
   + Creata en su implementación el objeto ``TransactionManager``.
   + Implementara él mismo métodos para crear transacciones y obtener la
     conexión asociada que se apoyaran en los correspondientes del gestor de
     transacciones, Eso permitiría prescindir del uso directo de
     ``TransactionManager`` en el resto del código.

   De este modo, sólo necesitaríamos usar esta clase ``Conexion``:

   .. code-block:: java

      Conexion conexion = Conexion.create("MiBase", dbUrl, dbUser, dbPassword);
      Centro centroDao = new CentroDao("MiBase");
