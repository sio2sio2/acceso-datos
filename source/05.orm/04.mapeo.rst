...
``@Lob``
   Se utiliza para campos que contiene cantidades apreciables de datos (``BLOB``
...
**Relación 1:1**
   En este caso, el campo se puede colocar en cualquiera de las dos tablas.
   Para significar este tipo relaciones se usa la notación ``@OneToOne``.

**Relación 1:N**
   En este caso, el campo se coloca en la tabla cuyos registros se relacionan
   con uno y sólo un registro de la otra tabla. Para significarla se usan las
   anotaciones ``@OneToMany`` y ``@ManyToOne``, ya veremos por qué ambas.

**Relación N:M**
   En el modelo relacional esta relación se traduce en un tabla aparte. Para
   significarla se usa la anotación ``@ManyToMany``.

Un detalle muy importante que debemos tener presente al anotar es que la
relación entre clases debe reflejarse en ambos extremos, esto es, en las dos
clases. Por eso, la relación **1:N** entre *Centro* y *Estudiante* provoca que
definamos así la clase ``Estudiante``:
...
Una relación **1:1** se define de forma análoga. Por ejemplo, si introdujéramos
una clase ``Direccion``, cada uno de cuyos objetos se asocia a un objeto
``Centro`` la relación sería **1:1** y podríamos definirla así en ``Centro``:
...
Este sería el extremo en donde que tendría representación en la base de datos.
En el otro extremo de la relación, la clase ``Direccion``:
...
La adición de *setters* y *getters* a las clases es la misma que la indicada en
a relación **1:N**.
