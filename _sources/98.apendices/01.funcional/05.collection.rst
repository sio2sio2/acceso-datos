...
.. note::
   ...
   Las colecciones, en realidad, están disponibles desde muy
...
.. note:: Para convertir un *array* existente en una lista puede usarse
...
.. attention:: No todas las colecciones son mutables, por lo que con aquellas
   que no lo sean no será posible usar los métodos que alteran su contenido
   (``add``, ``remove``, etc.)
...
``add(int i, E e)``
   que añade un nuevo elemento en la posición especificada.
...
:java-util:`ArrayList <ArrayList>`
   ...
   .. note:: Obsérvese que no se ha definido la variable ``al`` como
      ``ArrayList``, sino como ``List``. Esto significa que tendremos disponibles
      los métodos definidos en la interfaz :java-util:`List <List>`, pero no los
      que añade la clase :java-util:`ArrayList <ArrayList>`.
...
.. hint::
   ...
   Cuando se define el método de una clase es conveniente que los
