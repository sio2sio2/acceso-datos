...
.. admonition:: Advertencia
   ...
   y ``@OneToOne`` cargan inmediatamente la entidad relacionada, mientras que
   las relaciones ``@OneToMany`` y ``@ManyToMany`` lo hacen de forma perezosa:
...
.. tip:: La carga predefinida de las relaciones ``@*One`` es *perezosa*,
   mientras la de las relaciones ``@*Many`` es *inmediata*.
...
comportamiento derivado de las anotaciones, mientras que los *JOIN FETCH*
...
sustituyan por completo\ [#]_ a estas anotaciones. Para entender cómo crear
estos archivos es indispensable que entendamos que las anotaciones eran de dos
...
----------------------
El |ORM| busca para la definición del mapeo un archivo llamado
:file:`META-INF/orm.xml`. Si lo llamáramos de otro modo, entonces deberíamos
indicar en la configuración de la unidad de persistencia
(:file:`META-INF/persistence.xml`) cuál será su nombre:
...
Sea como sea, en el archivo podemos definir el mapeo más o menos así:
...
.. [#] O casi. En las últimas versiones de Hibernate_, el :ref:`procesador de
   anotaciones <orm-jpa-metamodel>` no es capaz de detectar las clases para las
   que debe generar el metamodelo si no se han anotado con ``@Entity``. Véase
   `el bug <https://hibernate.atlassian.net/browse/HHH-18722>`_.
