...
+ Los atributos que son campos se notan con ``@Column`` y pueden añadirse
  parámetros afinar la definición (nombre del campo si no coincide con el del
  atributo, restricciones adicionales, longitud si es una cadena). Un atributo
  sin la anotación, no obstante, se añadirá a la tabla como campo, a menos que
  sea ``static`` o ``transient``. Si se quiere forzar su exclusión, puede usarse
...
* Las transacciones no se cierra explícitamente, sino que se consideran acabadas
