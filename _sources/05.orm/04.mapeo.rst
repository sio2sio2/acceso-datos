...
``@Basic``/``@Transient``
   ...
   mientras que ``@Basic`` refuerza que se mape y puede añadir, adicionalmente,
   dos parámetros que ayudan a modificar el comportamiento predeterminado:

   - ``optional``, que puede ser ``true`` o ``false`` y permite indicar si la
     aplicación admite un valor nulo para el campo.
   - ``fetch``, que permite definir si se obtiene el campo al pedir el objeto
     (``EAGER``) o al pedir el campo usando el *getter* correspondiente (``LAZY``).
     Tiene sentido en campos que sean muy pesados como un ``BLOB``.
...
``@Column``
   ...
   + ``nullable``, que dependiendo de su valor *booleano* añade o no ``NOT
     NULL`` a la definición del campo.
...
.. note:: Tanto ``@Column`` como ``@Basic`` permiten indicar si un valor puede
   ser nulo. El primero es una pura indicación de |DDL| para el |SGBD| y, por lo
   tanto, la aplicación no sabe absolutamente nada sobre la validez del valor
   nulo. El segundo, en cambio, sí informa a la aplicación, aunque es muy común
   que los |ORM| se limiten a generar un ``NOT NULL`` en la definición del campo
   y el error sólo salte cuando el |SGBD| se niegue a guardar el valor inválido.
...
``@Version``
   ...
   .. note:: Como de la actualización de este campo debe encargarse |JPA|,
      nunca debería definirse un *setter* público.
...
.. attention:: Aunque hemos relacionado estas restricciones con el ``CHECK`` de
...
**Relación 1:N**: ``@OneToMany``/``@ManyToOne``
   ...
   **Unidireccional**
      ...
      **1:N** entre *Centro* y *Estudiante* provoca que definamos así la clase
      ...
      + No se ha especificado si la entidad relacionada (objetos ``Centro``) se
        carga de forma inmediata o perezosa. El comportamiento predeterminado en
        este lado de la relación es *EAGER*, esto es, carga inmediata (véase
        :ref:`optimización <orm-optimo>` para más información).
   ...
   **Bidireccional**
      ...
      *getter* para poder acceder a su valor. El *setter*, en cambio, no es
      ...
      A diferencia de lo que ocurre en el otro extremo, en este caso el
      comportamiento predeterminado en la entidad padre es *LAZY*.
...
**Relación 1:1**: ``@OneToOne``
   ...
   El comportamiento predeterminado para la carga en este tipo de relaciones es
   siempre *EAGER* sea cual sea el extremo.
...
**Relación N:M**:
   ...
   **N:M**. Para implementar en |JPA| estas relaciones, han de distinguirse
   ...
   a. La relación no tiene atributos propios: ``@ManyToMany``.
      ...
      la anotación ``@ManyToMany``, y la carga, a menos que se indique lo
      contrario, es siempre perezosa, sea cual sea el extremo.
