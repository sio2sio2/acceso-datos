...
2. Creación de un *Builder* (|JsonMapper.Builder|), en la que podremos añadir como más adelante
   veremos módulos, características generales de serialización (|SerializationFeature|) y
   deserialización (|DeserializationFeature|), y complementar la
   definición las clases del modelo con la técnica del *MixIn*.
...
.. code-block:: java
   ...
   // JsonMapper.Builder o MapperBuilder<?, ?>, que es la clase padre.
   MapperBuilder<?, ?> builder = JsonMapper.builder(factory)
    ...
    // JsonMapper o ObjectMapper, que es la clase padre.
...
**Enumeraciones** (:java-lang:`Enum`),
  si se usan las constantes de enumeración, esto es, si la enumeración es
  ...
  y se pretenden usar las cadenas *UNO* y *DOS* en el |JSON|.
