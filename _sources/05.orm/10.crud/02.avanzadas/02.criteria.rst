...
``.select`` a *estudiante* sin más. Que la consulta se haya hecho sobre Estudiante
...
.. tip:: En este último caso, puede usarse también :ref:`Tuple como en JPQL
   ...
   .. code-block:: java
      ...
      CriteriaQuery<Tuple> criteria = cb.createQuery(Tuple.class); // o cb.createTupleQuery();
      Root<Estudiante> estudiante = criteria.from(Estudiante.class);
      var nombre = estudiante.get("nombre").alias("nombre");
      var centro = estudiante.get("centro").alias("centro")
      criteria.multiselect(nombre, centro);

      TypedQuery<Tuple> query = em.createQuery(query);

   La ventaja de esta última forma se ve al recuperar los datos del |Tuple|:

   .. code-block:: java

      query.getResultList().forEach(t -> {
         // No hace falta especificar el tipo con un cast o un segundo argumento.
         String nombre = t.get(nombre);
         Long centro = t.get(centro);
      });
