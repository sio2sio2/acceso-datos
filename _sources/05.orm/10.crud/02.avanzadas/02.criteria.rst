...
#. A diferencia de lo que ocurre en la consulta |SQL|, el primero de los
   ...
   ¿Por qué es implícito? Porque pedidos objetos ``Centro`` completos y para
   ello |JPA| no tiene más remedio que recurrir a la relación. Incluso aunque no
   pidamos objetos ``Centro``, si en la expresiones (por ejemplo, en una condición)
   incluimos algún atributo de ``Centro``, |JPA| no tendrá más remedio que
   internamente recurrir al *JOIN*.
