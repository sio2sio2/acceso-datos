...
.. [#] Con *SQLite* por ejemplo el método ``.execute`` sí será capaz de ejecutar
   todas las sentencias |SQL|; ``executeUpdate``, en cambio, no lo hará. Con
   otros |SGBD| el comportamiento puede variar.
