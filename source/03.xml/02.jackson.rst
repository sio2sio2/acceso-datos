...
#. Al margen de lo anterior, las anotaciones suelen ser necesarias en |XML|
   ...
   elementos, pero también mediante nodos atributo. En cambio, en el modelo de
...
#. |XML| introduce otra dificultad: en |JSON| una secuencia o es el elemento
   ...
   porque no existen las secuencias *per se*, lo que existen son los nodos
...
muestra en forma de nodo atributo, por lo que es necesario especificarlo usando
también ``@JacksonXmlProperty``. Por otro lado, la anotación ``@JsonFormat`` no
es ninguna novedad: ya la necesitamos en |JSON| para definir cómo escribir la
fecha como cadena.
...
+ Como en |XML| es importante (al menos para los nodos elementos el orden), nos
  ...
  .. attention:: Es probable que para ``Tutor`` necesitemos crear una clase
...
.. warning:: Si no hemos definido una gramática |XSD| para el |XML|, todo son
