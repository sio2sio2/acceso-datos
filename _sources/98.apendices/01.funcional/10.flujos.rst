...
.. code-block:: java
   ...
   st.count(); // 4
   st.count(); // Lo consumimos contando antes, así que se produce un IllegalStateException
...
``forEach(Consumer<T> action)``
   ...
   iterando con un bucle, podríamos interpretarlo como un iterable\ [#]_:
...
``map(Function<T, R> transformador)``
   que genera un nuevo flujo en el que los elementos se generan aplicando la
   función transformadora a cada elemento del flujo original.

``reduce(T identity, BinaryOperator<T> accumulator)``
   que obtiene un valor aplicando acumulativamente la operación proporcionada en
