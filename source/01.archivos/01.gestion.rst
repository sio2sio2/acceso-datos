...
.. attention:: *Java* tiene dos |API| distintas para llevar a cabo estas tareas: 
...
``relativize(Path parent)``
   ...
   .. code-block:: java
      ...
      dir.relativize(dir.getParent()); // .config
...
.. important:: Estas herramientas contruyen rutas, pero tales rutas no
   tienen por qué existir en el sistemas de archivos.
...
.. caution:: En los proyectos de *Java* suele existir un directorio
   ...
   directorio acaba en la raíz del *classpath* de la aplicación. Por tanto, el
   ...
   *classpath* puede no ser automática, si hemos incluido ``<resources>`` en
   el archivo :file:`pom.xml`. Vea un ejemplo de :ref:`cómo añadir archivos de
   recursos <add-resources>` en uno de los apéndices.
