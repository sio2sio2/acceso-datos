...
Esta estrategia no es la más corta, pero ilustra más claramente cuál es la
...
* No es necesario indicar la lista de argumentos entre paréntesis, porque sólo
  hay uno. En caso contrario si deberían haberse escritos (:code:`(arg1, arg2)`).
...
:java-function:`Function<T, R> <Function>`
   Permite la definición de funciones que toman un argumento de entrada (el
   genérico ``T``) y devuelven un valor (el genérico ``R``). Para la ejecución
...
.. [#] Posiblemente al lector más avispado no se le habrá pasado por alto que
   ...
   llamado ``vencedor`` y en cada una de sus subclases implementar este
