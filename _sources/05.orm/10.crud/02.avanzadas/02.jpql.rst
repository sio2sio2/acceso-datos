...
Al margen de ello, comporte muchas de las particularidades internas que ya se
trataron en *Criteria* |API|, puesto que estas particularidades son propias del
propio |JPA| y no del lenguaje concreto de consulta que se utilice, por lo que
bajo este epígrafe no se será tan puntual en las explicaciones.
...
.. code-block:: java
   ...
   TypedQuery<Centro> query = em.createQuery("SELECT c FROM Centro c", Centro.class);
...
.. note:: Para esta consulta tan simple, se puede usar la cadena abreviada
   ":code:`FROM Centro`"
...
.. note:: Para poder agrupar por centro, *Java* debe conocer si dos centros son
   iguales. Esto podría tentarnos a haber definido en la clase ``Centro`` los
   métodos ``hashCode()`` y ``equals()``, pero no es necesario porque  |JPA|
   para las entidades que maneja las considera iguales cuando sus
   identificadores son iguales. De hecho, la consulta interna |SQL| agrupará por
   el identificador del centro.

.. note::
   :collapsible: closed

   Obsérvese que al estar consultando ``Estudiante``, los centros sin
